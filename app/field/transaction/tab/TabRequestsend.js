...
import { Text, StyleSheet, SectionList, Image, ScrollView, RefreshControl } from 'react-native'
...
export default class TabRequestsend extends Component {
    ...
    getAllData() {
        ...
        axios.get(`${config.uri}/data/transactions?where=status%3D'pending'&props=name%2Caddress%2Ccreated%2CdeadlineDate&loadRelations=typeOfShipping`).then((transactions) => {
            this.setState({ transactions: getDateSections(transactions.data, 'deadlineDate'), refreshing: false })
        })
    }
                                        ...
                                        <View style={{ flexDirection: 'row', padding: 10 }}>
                                            ...
                                            <View style={{ flex: 1, paddingLeft: 10 }}>
                                                ...
                                                <View style={{ flexDirection: 'row' }}>
                                                    <Text style={styles.rowTextDate}>{moment(item.created).format('L')}</Text>
                                                    <Text style={styles.rowTextSender}>{item.typeOfShipping.name}</Text>
                                                </View>
                                            </View>
                                        </View>
                                    )}
