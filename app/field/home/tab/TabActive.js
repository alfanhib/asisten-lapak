      ...
      if (result) {
        axios
          .get(
            `${
              config.uri
            }/data/stores?where=status%3D'active'%20and%20assistant_outdoor.objectId%3D'${result}'&props=name%2Caddress%2Clogo&loadRelations=assistant_cs`
          )
          .then(stores => {
    ...
    return (
      <Container>
        ...
        <Fab
          ...
          onPress={() => this.props.navigation.navigate("FieldHomeAddStore")}
        >
          <Icon name="add" />
        </Fab>
      </Container>
